...
The Flutter Orchestrator architecture is designed to be **Testable**. Business logic resides in pure Dart Executors, independent of Flutter → Simple and fast testing.
...
### 3.1. Setup with Scoped Bus

**Important:** Always use `SignalBus.scoped()` for test isolation.
...
Full flow test: Orchestrator → Dispatcher → Executor → Events.
...
- **Use Scoped Bus:** Always `SignalBus.scoped()` in tests
- **Dispose after test:** Call `dispose()` in `tearDown`
- **Test happy path & error cases:** Full scenarios
- **Mock external dependencies:** API, Database, etc.
- **Verify interactions:** Use `verify()` from Mocktail
